...
export class CustomValidators {
  ...
  public static customEmail(control: FormControl): ValidatorResult {
    return control.value.match(/(?!(?:.*\.){5})[a-zA-Z0-9.]+@[a-zA-Z0-9]{0,5}\.(com|org|net|co|us)/g)?.toString() === control.value ? null : {email: true};
  }


  public static customPassword(control: FormControl): ValidatorResult {
    return control.value.match(/(?=.*[0-9])(?=.*[!$%.-])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!$%.-]{5,}/g) ? null : {invalidPass: true};
  }

  public static customUsername(control: FormControl): ValidatorResult {
